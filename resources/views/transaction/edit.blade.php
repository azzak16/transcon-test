    ...
    <div class="card col-md-8">
        <form id="form" action="" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="transaction_no" value="{{ $datas->no_transaction }}">
            </div>
            <div class="form-group">
                <label for="transaction_date">Transaction Date</label>
                <input type="date" class="form-control" name="transaction_date" value="{{ $datas->transaction_date }}">
            </div>
            <div class="row">
                <P>Detail Items</P>
                <a href="#" id="tambah" class="btn btn-warning">add item</a>
            </div>
            <div class="items">
                @if ($datas->transactionDetails)
                    @foreach ($datas->transactionDetails as $items)
                        <div class="group">
                            <a href="#" class="btn btn-danger float-right hapus-isi">X</a>
                            <input type="hidden" value="{{ $items->id }}" name="id[]" class="id">
                            <div class="form-group">
                                <label for="item_name">Item Name</label>
                                <input type="text" class="form-control item_name" name="item_name[]" value="{{ $items->item }}">
                            </div>
                            <div class="form-group">
                                <label for="quantity">Quantity</label>
                                <input type="number" class="form-control quantity"  name="quantity[]" value="{{ $items->quantity }}">
                            </div>
                        </div>
                    @endforeach
                @endif
